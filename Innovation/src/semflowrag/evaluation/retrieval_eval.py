...
class RetrievalRecall(BaseMetric):
    ...
    def calculate_metric_scores(self, gold_docs: List[List[str]], retrieved_docs: List[List[str]], k_list: List[int] = [1, 5, 10, 20]) -> Tuple[Dict[str, float], List[Dict[str, float]]]:
        """
        Calculates Recall@k, Precision@k, and F1@k for each example and pools
        ...
        Args:
            ...
            k_list (List[int]): List of k values to calculate retrieval metrics for.

        Returns:
            Tuple[Dict[str, float], List[Dict[str, float]]]: 
                - A pooled dictionary with the averaged retrieval metrics across all examples.
                - A list of dictionaries with retrieval metrics for each example.
        """
        ...
        for example_gold_docs, example_retrieved_docs in zip(gold_docs, retrieved_docs):
            ...
            gold_doc_set = set(example_gold_docs)
            # Compute retrieval metrics for each k
            for k in k_list:
                # Get top-k retrieved documents
                top_k_docs = example_retrieved_docs[:k]
                # Calculate intersection with gold documents
                relevant_retrieved = set(top_k_docs) & gold_doc_set
                num_relevant_retrieved = len(relevant_retrieved)

                recall = num_relevant_retrieved / len(gold_doc_set) if gold_doc_set else 0.0
                precision = num_relevant_retrieved / len(top_k_docs) if top_k_docs else 0.0
                f1 = 2 * precision * recall / (precision + recall) if precision + recall > 0 else 0.0

                example_eval_result[f"Recall@{k}"] = recall
                example_eval_result[f"Precision@{k}"] = precision
            ...
            for k in k_list:
                for metric_name in metric_names:
                    ...
                    pooled_eval_results[metric_key] += example_eval_result[metric_key]

        # Average pooled results over all examples
        num_examples = len(gold_docs)
        for k in k_list:
            for metric_name in metric_names:
                pooled_eval_results[f"{metric_name}@{k}"] /= num_examples
