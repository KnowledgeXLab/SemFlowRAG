...
from src.semflowrag.SemFlowRAG import SemFlowRAG
from src.semflowrag.StandardRAG import StandardRAG
from src.semflowrag.utils.misc_utils import string_to_bool
from src.semflowrag.utils.config_utils import BaseConfig
from src.semflowrag.embedding_store import EmbeddingStore
...
INNOVATION_DIR = Path(__file__).resolve().parent
DATASET_DIR = INNOVATION_DIR / "reproduce" / "dataset"
RERANK_DSPY_FILE = (
    INNOVATION_DIR
    / "src"
    / "semflowrag"
...
def main():
    parser = argparse.ArgumentParser(description="SemFlowRAG retrieval and QA")
    ...
    samples = json.load(open(DATASET_DIR / f"{dataset_name}.json", "r"))
    ...
    config = BaseConfig(
        ...
        rerank_dspy_file_path=str(RERANK_DSPY_FILE),
        retrieval_top_k=200,
        linking_top_k=5,
        max_qa_steps=3,
        qa_top_k=5,
    ...
    semflowrag = StandardRAG(global_config=config)
    # semflowrag = SemFlowRAG(global_config=config)

    semflowrag.index(docs)

    # Retrieval and QA
    semflowrag.rag_qa(queries=all_queries, gold_docs=gold_docs, gold_answers=gold_answers)
