...
def truncated_min_max_normalize(x, lo_q: float = 0.01, hi_q: float = 0.99):
    """Percentile-truncated Min-Max. Values below the lo_q quantile clip to 0, above hi_q clip to 1.

    Used for the directed-PPR innovation so that outliers in per-entity relevance / diversity / edge
    priors don't compress the bulk of the distribution against the scale. Returns a float32 array in [0, 1].
    """
    ...
    if hi <= lo:
        # Degenerate distribution — fall back to uniform 0.5 so downstream math stays well-defined.
