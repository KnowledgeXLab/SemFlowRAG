...
- Extract as many explicit factual statements as possible from the paragraph; aim for high recall rather than a small summary.
- Each triple should contain at least one named entity when possible, but do not omit explicit facts whose useful endpoints are descriptive phrases, dates, quantities, titles, roles, events, locations, affiliations, or other salient noun phrases.
- Split enumerations and compound statements into separate triples when they express multiple facts.
- Preserve factual specificity from the paragraph, including aliases, dates, numbers, locations, occupations, memberships, affiliations, works, events, and descriptive attributes.
- Do not infer facts that are not explicitly supported by the paragraph.
